# SPMS GitHub-Based Deployment Script for PowerShell (Simple Version)
...
if ($Changes) {
    ...
    $CommitMsg = "DEPLOYMENT v$(Get-Date -Format 'yyyyMMdd_HHmmss'): GitHub-based deployment"
...
# Confirm deployment
Write-Host "WARNING: This will deploy SPMS via GitHub to Ubuntu production server" -ForegroundColor Yellow
...
ssh "$VpsUser@$VpsHost" "cd $VpsPath && if [ ! -d .git ]; then echo 'Initializing git repository...'; git init; git remote add origin $GitRemote; fi && echo 'Fetching latest changes from GitHub...' && git fetch origin main && echo 'Resetting to latest version...' && git reset --hard origin/main && echo 'Latest code pulled successfully'"
