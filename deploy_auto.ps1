...
ssh "$VpsUser@$VpsHost" "cd /var/www && sudo rm -rf spms && git config --global --add safe.directory $VpsPath && echo 'Cloning fresh from GitHub...' && sudo git clone $GitRemote spms && sudo chown -R spms:spms $VpsPath && echo 'Repository cloned successfully' && cd $VpsPath && echo `"Current commit: `$(git log -1 --oneline)`""
